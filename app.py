...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    ...
    for i, (frame, mask) in enumerate(
        zip(video_state["origin_images"], video_state["masks"])
    ):
        frame = frame.copy()
        ...
        frame_cropped = frame_masked[
            rect[1] : rect[1] + rect[3], rect[0] : rect[0] + rect[2]
        ]
        if (
            frame_cropped is None
            or len(frame_cropped) < 1
            or frame_cropped.shape[0] < 1
            or frame_cropped.shape[1] < 1
        ):
            continue
        try:
